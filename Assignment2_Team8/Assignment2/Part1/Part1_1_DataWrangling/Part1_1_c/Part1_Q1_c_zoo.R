...
mergeddata$value[mergeddata$value == 0] <- NA
View(mergeddata)

##------to replace zero values used na.locf from zoo library
mergesdata<- zoo(mergeddata$value)
X<-na.locf(mergesdata, na.rm=TRUE)
y<-data.frame(X)
View(y)

final<-cbind(mergeddata[2:14],mergeddata$Temperature)
View(final)
newdataformat<-cbind(final[2:3],KWh=y$X,month=final$ordereddata_month,day = final$ordereddata_day,year= final$ordereddata_year,hour= final$Hour,DayofWeek = final$ordereddata_weekday, WeekDay = final$ordereddata_weekend, Peakhour = final$ordereddata_peakhours,Temperature = mergeddata$Temperature) 
View(newdataformat)
write.csv(newdataformat,file="Part1_Q1_c_nareplaced.csv",row.names = FALSE)